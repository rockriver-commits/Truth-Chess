  ...
  return (
    <div className="w-full max-w-[600px] mx-auto mt-3 rounded-2xl bg-stone-100 ring-1 ring-stone-200 p-2.5 shadow-sm">
      <div className="flex items-center justify-center gap-2">
        <button type="button" onClick={onFirst} disabled={atFirst} title="First move"
          className={`${base} ${grey} h-9 w-9`}>
          <SkipBack className="h-4 w-4" />
        </button>
        <button type="button" onClick={onPrev} disabled={atFirst} title="Previous move"
        ...
        <button type="button" onClick={onNext} disabled={atLast} title="Next move"
          className={`${base} ${orange} h-9 px-4 gap-1`}>
          <span className="text-sm">Next</span>
          <ChevronRight className="h-4 w-4" />
        </button>
        <button type="button" onClick={onLast} disabled={atLast} title="Last move"
          className={`${base} ${grey} h-9 w-9`}>
          <SkipForward className="h-4 w-4" />
        </button>
      </div>
      <button type="button" onClick={onLive} disabled={atLive} title="Live"
