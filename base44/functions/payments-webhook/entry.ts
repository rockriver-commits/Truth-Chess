...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant Pro access by tagging the user's `plan`. Idempotent: setting the same
  // value twice is safe. Prefer the signed-in app user (purchase.appUserId);
  // for an anonymous buyer fall back to matching buyerEmail against a User
  // record (User records are invite-only and cannot be created here).
  if (purchase.productId === "pro_monthly") {
    if (purchase.appUserId) {
      ...
      if (users?.[0]) await db.entities.User.update(users[0].id, { plan: "pro" });
    }
  }
  // ===== END APP-SPECIFIC =====
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke Pro access (mirror of the grant). Idempotent.
  if (purchase.productId === "pro_monthly") {
    if (purchase.appUserId) {
      ...
      if (users?.[0]) await db.entities.User.update(users[0].id, { plan: "free" });
    }
  }
  // ===== END APP-SPECIFIC =====
